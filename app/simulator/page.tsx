...
import { Settings2, Cpu, BarChart3, AlertCircle, ChevronDown, Activity } from "lucide-react";
...
// Urutan standar: EXP -> Jungle -> Mid -> Gold -> Roam
...
export default function Home() {
  const [teamA, setTeamA] = useState(MPL_TEAMS[0]);
  const [teamB, setTeamB] = useState(MPL_TEAMS[1]);
  const [isSelectorOpen, setIsSelectorOpen] = useState(false);

  // --- STATE DRAFT DENGAN UNIQUE ID (uid) ---
  // Penambahan `uid` ini yang bakal bikin Framer Motion paham kalau kartu cuma "geser", bukan dirender ulang
  ...
  const reshuffleByLaneOrder = useCallback((picks: any[], targetOrder: string[]) => {
    ...
    const leftover = [...picks]; // Bawa objek asli berserta uid-nya

    targetOrder.forEach((laneName, idx) => {
      // Cari hero yang rolenya cocok sama lane target
      ...
      if (matchIdx !== -1) {
        // Pindahkan kartu asli ke urutan baru
        sorted[idx] = leftover.splice(matchIdx, 1)[0];
        sorted[idx].lane = laneName; // Sesuaikan label lane
      }
    });

    // Isi sisa slot dengan hero yang sisa (kalau drafnya unik/nyeleneh)
  ...
  const handleSelect = (heroId: string) => {
    ...
    if (currentAction?.type === "pick") {
      ...
      setPicks(prev => {
        ...
        // Pertahankan uid dan lane, cuma ganti heroId
    ...
    // Auto-Reshuffle setelah hero terakhir ke-lock
    if (nextStep === MPL_DRAFT_FLOW.length) {
      ...
      }, 500); // Jeda bentar biar user lihat pick ke-10 masuk dulu
    }
  };
  ...
  const handleAnalyzeDraft = async () => {
    ...
    // Safety check reshuffle ulang (kalau belum)
    ...
    try {
      ...
      console.error(error);
      alert("Error: Hubungkan backend ml-service di port 8001.");
        ...
        <div className="w-full flex justify-center items-center gap-10 sm:gap-20 mb-12 mt-4 px-4">
          ...
          <div className="flex flex-col gap-6 items-start">
            <div className="flex gap-2 sm:gap-4">
              {/* WAJIB PAKE p.uid SEBAGAI KEY BIAR ANIMASINYA MULUS! */}
        ...
        <div className="w-full max-w-5xl px-6 flex flex-col items-center">
          <AnimatePresence mode="wait">
            {!isFinished ? (
              ...
              <motion.div key="analyze-section" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center w-full">
                {!prediction ? (
                  <button onClick={handleAnalyzeDraft} disabled={isAnalyzing} className="px-14 py-6 rounded-2xl bg-[#09090b] border border-white/20 text-white font-black tracking-widest uppercase hover:border-white/50 transition-all shadow-2xl">
                    <span className="flex items-center gap-4">
                      {isAnalyzing ? <Activity className="animate-pulse" /> : <Cpu />}
                      {isAnalyzing ? "Processing Telemetry..." : "Execute Analysis"}
                    </span>
                  </button>
                ) : (
                  <div className="w-full mt-8 p-10 rounded-3xl bg-[#0a0a0c]/80 border border-white/10 backdrop-blur-2xl">
                    <div className="flex items-center gap-3 mb-8">
                      <BarChart3 className="text-blue-400" />
                      <h3 className="text-2xl font-black uppercase">Prediction Outcome</h3>
                    </div>
                    {/* Progress Bar & Insights */}
                    <div className="mb-10">
                       <div className="flex justify-between mb-4 text-xs font-black uppercase">
                         <span className="text-blue-400">{teamA.name}</span>
                         <span className="text-red-400">{teamB.name}</span>
                       </div>
                       <div className="w-full h-10 rounded-lg bg-black overflow-hidden flex">
                         <motion.div initial={{ width: 0 }} animate={{ width: `${prediction.winProbability.teamA * 100}%` }} className="bg-blue-600 flex items-center px-4 font-black">{(prediction.winProbability.teamA * 100).toFixed(0)}%</motion.div>
                         <div className="flex-1 bg-red-600 flex items-center justify-end px-4 font-black">{(prediction.winProbability.teamB * 100).toFixed(0)}%</div>
                       </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <div className="p-6 rounded-xl bg-white/5 border border-white/5">
                         <h4 className="text-[10px] font-black text-white/40 uppercase mb-3">Tactical Insight</h4>
                         <p className="text-sm text-white/80">{prediction.insight}</p>
                       </div>
                       <div className="p-6 rounded-xl bg-white/5 border border-white/5">
                         <h4 className="text-[10px] font-black text-white/40 uppercase mb-3">Key Variables</h4>
                         <div className="flex flex-wrap gap-2">
                           {prediction.recommendations.map((r: string, i: number) => <span key={i} className="px-3 py-1 bg-white text-black text-[10px] font-black rounded">{r}</span>)}
                         </div>
                       </div>
                    </div>
                  </div>
